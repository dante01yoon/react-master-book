...
const DisplayComponent: React.FC<DisplayComponentProps> = ({ message }) => {
  return (
    <div style={{ border: '1px solid #eee', padding: '10px', marginTop: '10px' }}>
      <h4>표시 컴포넌트 (자식)</h4>
      <p>수신된 메시지: <strong>{message}</strong></p> {/* 부모로부터 받은 메시지를 화면에 표시함 */}
    </div>
  );
};
...
const InputComponent: React.FC<InputComponentProps> = ({ onMessageChange }) => {
  const [inputValue, setInputValue] = useState(''); // 내부 입력 필드 상태

  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setInputValue(event.target.value); // 입력 값 변경 시 내부 상태 업데이트함
  };
  ...
  return (
    <div style={{ border: '1px solid #eee', padding: '10px', marginTop: '10px' }}>
      ...
      />
      <button onClick={handleSubmit} style={{ padding: '5px 10px' }}>메시지 업데이트 요청</button> {/* 버튼 클릭 시 handleSubmit 호출함 */}
    </div>
  );
};

// 부모 컴포넌트: 상태를 관리하고 자식들에게 데이터를 전달하며, 자식으로부터 상태 변경 요청을 받음
const UnidirectionalDataFlowExample = () => {
  // currentMessage는 이 컴포넌트가 소유하고 관리하는 상태(state)임
  // 이 상태가 UI를 표현하는 데이터의 주체가 됨
  const [currentMessage, setCurrentMessage] = useState("초기 메시지입니다.");

  // 자식 컴포넌트(InputComponent)로부터 호출될 콜백 함수
  // 이 함수가 호출되면 ParentComponent의 상태(currentMessage)가 업데이트됨
  // 상태 변경은 항상 상태를 소유한 부모 컴포넌트에서 관리됨
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif', border: '2px solid #007bff', borderRadius: '8px', backgroundColor: '#f0f8ff' }}>
      <h2>단방향 데이터 흐름 및 단방향 바인딩 예제 (부모 컴포넌트)</h2>
      {/*
      ...
      */}
      ...
      {/*
        InputComponent는 사용자의 입력을 받아 ParentComponent의 상태를 변경하도록 "요청"함.
        ...
        하위 컴포넌트로 전파되는 단방향 흐름을 명확히 보여줌 (액션/이벤트 흐름: 자식 -> 부모, 상태 업데이트 후 데이터 흐름: 부모 -> 자식).
      */}
...
// 애플리케이션의 다른 부분에서 이 컴포넌트를 사용하려면 아래와 같이 임포트하여 사용함
// import UnidirectionalDataFlowExample from './state/UnidirectionalDataFlowExample';
//
// function App() {
//   return (
//     <div className="App">
//       <UnidirectionalDataFlowExample />
//     </div>
//   );
// }
//
