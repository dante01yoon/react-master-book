...
// 잘못된 예시: 프롭으로 받은 객체를 직접 수정함 (불변성 위반)
// 이런 방식은 React에서 문제를 일으킬 수 있음
const GoldenRabbitWrongButton = ({ info }) => {
  const handleClick = () => {
    // 직접 수정 (X) -> 예측 불가능한 동작 및 리렌더링 문제 유발 가능
    info.status = "reveal"
  }
  return (
    <button onClick={handleClick} disabled={info.status !== "hide"}>
      Click to reveal the Golden Rabbit (Wrong Way)
    </button>
  );
};
